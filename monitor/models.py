...
class ChatMessage(models.Model):
    """
    Modelo para mensajes de chat entre usuarios.
    Los usuarios regulares solo pueden chatear con el root/admin.
    El root puede iniciar chat con cualquier usuario.
    """
    ...
    def __str__(self):
        return f"{self.sender.email} → {self.recipient.email} ({self.created_at:%Y-%m-%d %H:%M})"
